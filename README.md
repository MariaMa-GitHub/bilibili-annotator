...
- **视频库** — 在一个页面浏览全部已标注视频（仅展示至少含有一条标注、摘要、评分或点赞/点踩的视频）。可按标题、摘要、标注内容和标签搜索；通过三个独立筛选控件过滤（标签多选下拉、👍/👎 切换按钮、星级评分下拉）；按最近观看、添加日期、标注数量或评分排序（同分时按标题字母顺序）。卡片显示进度条、精确时间戳（`X% · M:SS / M:SS`）和上次观看日期，超过 20 条时分页展示。
