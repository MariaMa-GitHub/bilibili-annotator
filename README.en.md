...
- **Dashboard** — Browse all annotated videos in one page (only videos with at least one annotation, summary, rating, or like/dislike are shown). Search across titles, summaries, annotation text, and tags. Filter using three independent controls: a multi-select tag dropdown, 👍/👎 toggle buttons, and a star-rating dropdown. Sort by last watched, date added, annotation count, or rating (ties broken alphabetically). Cards show a progress bar, exact timestamp (`X% · MM:SS / MM:SS`), and last-watched date. Paginated at 20 per page.
